...
export interface RescheduleControlProps {
  serviceId: string;
  /** Perform the reschedule; return true on success. */
  onConfirm: (startIso: string) => Promise<boolean>;
  /** Slot-grid columns at ≥sm (staff view is wider). */
  columns?: 4 | 5;
  /** Trigger-button alignment within its row. */
  triggerAlign?: "start" | "end";
}

/**
 * Inline "reschedule" control for one appointment: toggles a slot picker (the
 * same day/time UI as the booking wizard) and moves the appointment to a new
 * slot. Zone-agnostic — the caller supplies the server action via `onConfirm`.
 */
export function RescheduleControl({
  ...
  async function confirm() {
    ...
    setError(null);
    try {
      const ok = await onConfirm(slot.startIso);
      if (ok) {
        setOpen(false);
        router.refresh();
      } else {
  ...
  // Static class strings so Tailwind picks up both variants.
  const slotGrid =
    columns === 5
  ...
  return (
    <div className="space-y-3">
      <div className={triggerAlign === "end" ? "text-right" : undefined}>
          ...
          {!loading && days && days.length > 0 && (
            <>
              ...
              <div className="flex justify-end">
                <Button size="sm" disabled={!slot || submitting} onClick={confirm}>
